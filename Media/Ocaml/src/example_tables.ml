...
let students : table = {
  schema = [col "name" String; col "age" Int32; col "favorite color" String];
  rows = [
...
let employees : table = {
  schema = [col "Last Name" String; col "Department ID" Int32];
  rows = [
...
let departments : table = {
  schema = [col "Department ID" Int32; col "Department Name" String];
  rows = [
...
let jelly_anon : table = {
  schema = List.map (fun name -> col name Bool)
...
let jelly_named : table = {
  schema = col "name" String :: jelly_anon.schema;
...
let gradebook : table = {
  schema = List.map (fun name -> col name (if name = "name" then String else Int32))
...
let gradebook_seq : table = {
  schema = [col "name" String; col "age" Int32; col "quizzes" (Sequence (List.to_seq [Int32])); col "midterm" Int32; col "final" Int32];
  rows = [
...
let quiz_table grades : table = {
  schema = [col "quiz#" Int32; col "grade" Int32];
  rows = List.mapi (fun index grade -> ["quiz#", Int (index + 1); "grade", Int grade]) grades;
}

let gradebook_table : table = {
  schema = [col "name" String; col "age" Int32; col "quizzes" Table; col "midterm" Int32; col "final" Int32];
  rows = [
